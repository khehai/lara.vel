  ...
  <div class="space-y-1 text-center" x-data="showImage()">
    <div class="flex-1 flex-wrap">
      <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
       ...
       <img id="preview" class="object-cover h-32 mt-2 w-60 text-center" @if ($value) src="/storage/{{$value}}" @endif >
    </div>
    <div class="flex text-sm text-gray-600">
      <label for="file-upload" class="relative cursor-pointer rounded-md bg-white font-medium text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:text-indigo-500">
        <span>Upload a file</span>
        <input id="file-upload" name="{{$name}}" type="file" class="hidden sr-only" @change="showPreview(event)">
      </label>
